...
public class BoardDao {
	...
	public List<BoardDto> searchList(String value1, String value2) throws Exception {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
			if (value1.equals("CTITLE")) {
				sql = "SELECT BNO, MNO, CTITLE, WMID, BMOD_DATE, BVIEWS, BREPLY" + " FROM SAMPLEBOARD"
						+ " WHERE CTITLE LIKE ?" + " ORDER BY BNO DESC";// '%입문자%'
			} else if (value1.equals("WMID")) {
				sql = "SELECT BNO, MNO, CTITLE, WMID, BMOD_DATE, BVIEWS, BREPLY" + " FROM SAMPLEBOARD"
						+ " WHERE WMID LIKE ?" + " ORDER BY BNO DESC";
			}
	...
	public BoardDto boardDetailView(int no, int hitCnt) throws Exception {
		...
		try {
			...
			if (hitCnt == 1) {
				sql = "UPDATE SAMPLEBOARD SET BVIEWS = BVIEWS+?" + " WHERE BNO = ?";
			...
			sql = "SELECT BNO, WMID, CTITLE, BCONTENTS" + " FROM SAMPLEBOARD" + " WHERE BNO = ?";
	...
	public int boardUpdate(BoardDto boardDto) throws Exception {
		...
		try {
			sql = "UPDATE SAMPLEBOARD SET CTITLE = ?, BCONTENTS = ?, BMOD_DATE = SYSDATE" + " WHERE BNO = ?";
